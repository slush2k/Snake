...
let SnakeMoveTickID: NodeJS.Timer;
//initArena
const arena = <HTMLCanvasElement>document.getElementById("canvas");
...
function rect(x: number, y: number, size: number) {
  //Draws Squares
  ctx.fillStyle = snake.colourBody;
  ctx.fillRect(x, y, size, size);
}

function tileErase(x: number, y: number) {
  //Erases a tile
...
function drawSegment(x: number, y: number) {
  ...
  ctx.fillStyle = snake.colourBody;
  ctx.fillRect(
    (x + bevelOffset) * tileWidth,
    (y + bevelOffset) * tileWidth,
    snake.width,
    snake.width
  );
}

function drawHead(x: number, y: number, dir: number) {
  //draw head
  drawSegment(x, y);
  //draw eyes
  ...
  switch (dir) {
    case Direction.Up:
      ctx.fillRect(
        (x + bevelOffset + 0.15) * tileWidth,
        (y + 0.15 + bevelOffset) * tileWidth,
        snake.width * 0.1,
        snake.width * 0.1
      );
      ctx.fillRect(
        (x + 1 - bevelOffset - 0.15) * tileWidth - snake.width * 0.1,
        (y + 0.15 + bevelOffset) * tileWidth,
        snake.width * 0.1,
        snake.width * 0.1
      );
    ...
    case Direction.Left:
      ctx.fillRect(
        (x + 0.15 + bevelOffset) * tileWidth,
        (y + bevelOffset + 0.15) * tileWidth,
        snake.width * 0.1,
        snake.width * 0.1
      );
      ctx.fillRect(
        (x + 0.15 + bevelOffset) * tileWidth,
        (y + 1 - bevelOffset - 0.15) * tileWidth - snake.width * 0.1,
        snake.width * 0.1,
        snake.width * 0.1
      );
      break;
...
function isAppleHere(x: number, y: number): boolean {
...
function UpdateMovementDirection(movement: number) {
...
function CollisionDetectionWall(): boolean {
  if (
    snakeSegments[0].xPos > gridDiv - 1 ||
...
function CollisionDetectionSnake(): boolean {
  for (let i = 1; i < snakeSegments.length; i++) {
    if (
      snakeSegments[0].xPos == snakeSegments[i].xPos &&
